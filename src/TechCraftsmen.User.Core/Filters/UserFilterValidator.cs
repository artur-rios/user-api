...
namespace TechCraftsmen.User.Core.Filters
{
    ...
    public class UserFilterValidator : FilterValidator
    {
        ...
        private readonly EntityFilter _roleIdFilter = new("RoleId", typeof(int), value => value.IsIntBiggerThanZero() && Enum.IsDefined(typeof(Roles), (int)value));
        ...
        private readonly Func<string, object> _activeFilterParser = (value) =>
        {
            var result = value.ToBool();

            return result is not null ?
                result.Value : throw new NotAllowedException("Could not parse to bool");
        };
        ...
        public override void SetFilters()
        {
            ...
            _activeFilter.Parser = _activeFilterParser;
